...
pub async fn check_cache(problem_id: &i32, input_sum: String, output_sum: String) -> Result<bool> {
	...
	{
		println!("{:?}, {:?}", input_sum, md5::compute(input2.trim().as_bytes()));
		return Ok(false);
	}
...
pub async fn cache_problem(creds: &HashMap<&str, String>, client: &reqwest::Client, url: &String, problem_id: i32) -> Result<()> {
	if Path::exists(Path::new(&format!("problems/{}", problem_id))) {
		fs::remove_dir_all(&format!("problems/{}", problem_id))
			.await.with_context(|| format!("Unable to clear dir for problem {}.", problem_id))?;
	}
	fs::create_dir(&format!("problems/{}", problem_id))
		.await.with_context(|| format!("Unable to create dir for problem {}.", problem_id))?;
	...
	return match res.status() {
		reqwest::StatusCode::OK => match res.json::<Problem>().await {
			Ok(parsed) => {
				fs::write(format!("problems/{}/input.txt", problem_id), &parsed.problem_input)
					.await.with_context(|| format!("Unable to write input for problem {}.", problem_id))?;
				fs::write(format!("problems/{}/output.txt", problem_id), &parsed.problem_output)
					.await.with_context(|| format!("Unable to write output for problem {}.", problem_id))?;
				Ok(())
			}
